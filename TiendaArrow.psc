Algoritmo TiendaArrow
			...
			SiNo
				...
				Si opcion_menup = 3 Entonces
					...
					Si opcion_accesorios >=1 Y opcion_accesorios <= 6 Entonces
						...
						Si opcion_seleccion = "S" O opcion_seleccion = "s" Entonces
							Escribir "Ingrese el número del producto que desea seleccionar: "
							Leer opcion_producto
							Escribir "¿Qué cantidad de ese producto quiere?: "
							Leer cantidad
							//calculo del total a pagar y del total de productos que hay en el carrito
							total <- total + (cantidad * precio)
							contador <- contador + cantidad
	...
	sino
		...
		//recargo del 5% si el método de pago es con tarjeta de crédito
		Si metodo = 3 Entonces
			recargo <- total * 0.05
			total <- total + recargo
		FinSi
		...
		//tiempo de pedido segun la distancia
		...
		Si recargo > 0 Entonces
			Escribir "Recargo por tarjeta (5%): Q", recargo
		FinSi
		Escribir "Total General: Q", total
		Escribir "Tiempo estimado de entrega: ", tiempo, " minutos"
